...
class Net(nn.Module):
    ...
    def __init__(self, attr_size, kernel_size = 3, num_filter = 32, pooling_method = 'attention', rnn = 'lstm'):
        ...
        print ("Model: super(Net, self).__init__()")
        ...
        print ("Model: if rnn == 'lstm':")
        if rnn == 'lstm':
            print ("Model: self.rnn = nn.LSTM: start")
            self.rnn = nn.LSTM(input_size = num_filter + 1 + attr_size, \
                                      hidden_size = 128, \
                                      num_layers = 2, \
                                      batch_first = True
            )
            print ("Model: self.rnn = nn.LSTM: End")
        elif rnn == 'rnn':
            print ("Model: self.rnn = nn.LSTM: start")
    ...
    def mean_pooling(self, hiddens, lens):
        print ("Model: mean_pooling(self, hiddens, lens)")
    ...
    def attent_pooling(self, hiddens, lens, attr_t):
        print ("Model: attent_pooling(self, hiddens, lens, attr_t)")
    ...
    def forward(self, traj, attr_t, config):
        print ("Model: forward(self, traj, attr_t, config):")
